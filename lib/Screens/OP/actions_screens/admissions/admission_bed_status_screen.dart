...
class AdmissionBedStatusScreen extends StatefulWidget {
  final String patientName;
  final String crn;
  const AdmissionBedStatusScreen({
    super.key,
    required this.patientName,
    required this.crn,
  });
  @override
  State<AdmissionBedStatusScreen> createState() =>
      _AdmissionBedStatusScreenState();
}

class _AdmissionBedStatusScreenState extends State<AdmissionBedStatusScreen> {
  ...
  void _showRedPopup(String uniqueId, int index) {
    ...
    // Assuming ~4 boxes per row on standard mobile width,
    // Box 0 & 1 -> Extend to Right (Tail on Left)
    // Box 2 & 3 -> Extend to Left (Tail on Right)
      ...
      builder: (context) => Stack(
        children: [
          ...
          CompositedTransformFollower(
            ...
            // Offset aligns the tail explicitly with the target center
      ...
      children: [
        ...
        Wrap(
          ...
          children: beds.asMap().entries.map((entry) {
            int index = entry.key; // Track the position
            var b = entry.value;
            ...
            return GestureDetector(
              onTap: () {
                ...
                if (b.color == const Color(0xFF1B8019)) {
                  ...
                  _showRedPopup(uniqueId, index); // Pass index to detect side
                }
              },
              child: bedWidget,
            );
